          ...
          <div className="flex justify-between items-center">
            ...
            {activeTab !== 'dashboard' && (
              <button
                ...
                className="bg-gradient-to-r from-green-600 to-green-700 text-white px-6 py-3 rounded-xl hover:from-green-700 hover:to-green-800 transition-all duration-200 flex items-center shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
              >
                <Plus size={20} className="mr-2" />
                Tambah {activeTab === 'articles' ? 'Artikel' : activeTab === 'members' ? 'Anggota' : 'Foto'}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          {activeTab === 'dashboard' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl p-6 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-blue-100 text-sm font-medium">Total Artikel</p>
          ...
          {activeTab !== 'dashboard' && (
            ...
            {activeTab === 'articles' && (
              <div className="space-y-4">
                {articles.map((article) => (
                  <div key={article.id} className="bg-gray-50 rounded-xl p-6 hover:bg-white hover:shadow-lg transition-all duration-200 border border-gray-100">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <h3 className="font-bold text-gray-900 text-lg mb-2">{article.judul}</h3>
                        <p className="text-sm text-gray-600 mb-3 line-clamp-2">{article.isi}</p>
            ...
            {activeTab === 'members' && (
              <div className="space-y-4">
                {members.map((member) => (
                  <div key={member.id} className="bg-gray-50 rounded-xl p-6 hover:bg-white hover:shadow-lg transition-all duration-200 border border-gray-100">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center space-x-4 flex-1">
                        ...
                        <div>
                          <h3 className="font-bold text-gray-900 text-lg">{member.nama}</h3>
                          <p className="text-green-600 font-medium">{member.jabatan}</p>
                          {member.kontak && (
                            <p className="text-sm text-gray-500 mt-1">{member.kontak}</p>
                          )}
                        </div>
                      </div>
                      <div className="flex space-x-2 ml-4">
                  ...
                  <div key={item.id} className="relative group">
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg flex items-center justify-center space-x-2">
                      <button
                        ...
                        className="p-2 text-white hover:bg-blue-600 rounded-lg transition-all duration-200"
                        title="Edit"
                      >
                        <Edit size={18} />
                      </button>
                      <button 
                        ...
                        className="p-2 text-white hover:bg-red-600 rounded-lg transition-all duration-200"
                        title="Hapus"
                      >
                        <Trash2 size={18} />
                      </button>
